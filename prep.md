...
-   Flexbox uses columns or rows, Grid can use both

### Difference between `display: inline` and `display: block` and `display: inline-block`

#### `display: inline`

-   Height and width cannot be defined they are adjusted to the content

#### `display: block`

-   Height and width can be defined
-   It starts on a new line
-   It occupies the full line space by default
-   It does not let anything else in the same line
